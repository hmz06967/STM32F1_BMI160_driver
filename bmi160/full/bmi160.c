...
static int8_t disable_sig_motion_int();
...
static int8_t enable_data_ready_int();
...
static int8_t map_read_len(uint16_t * len);
static int8_t config_sec_if();
#endif
...
static void reset_fifo_data_structure(void);
...
int8_t bmi160_get_regs(uint16_t reg_addr, uint8_t * data, uint16_t len) {
    ...
    if ((dev == NULL) || (dev -> read == NULL)) {
        ...
        rslt = dev -> read((dev -> id << 1), reg_addr, (uint8_t * ) data, len);
    }
    return rslt;
}
...
int8_t bmi160_set_regs(uint16_t reg_addr, uint8_t * data, uint16_t len) {
    ...
    if ((dev == NULL) || (dev -> write == NULL)) {
        ...
        if ((dev -> prev_accel_cfg.power == BMI160_ACCEL_NORMAL_MODE) || (dev -> prev_gyro_cfg.power == BMI160_GYRO_NORMAL_MODE)) {
            rslt = dev -> write((dev -> id << 1), reg_addr, data, len);
            dev -> delay_ms(5);
        } else {
            for (; count < len; count++) {
                rslt = dev -> write((dev -> id << 1), reg_addr, & data[count], len);
                reg_addr++;
                dev -> delay_ms(1);
            }
        }
...
int8_t bmi160_get_fifo_data() {
    ...
    if ((dev == NULL) || (dev -> fifo -> data == NULL)) {
        ...
        reset_fifo_data_structure();
        rslt = get_fifo_byte_counter( & bytes_to_read);
        if (rslt == BMI160_OK) {
            ...
            rslt = bmi160_get_regs(BMI160_FIFO_DATA_ADDR, dev -> fifo -> data, dev -> fifo -> length);
        }
    }
    return rslt;
}
int8_t bmi160_set_fifo_flush() {
    int8_t rslt = 0;
    ...
    if (dev == NULL) {
        ...
        rslt = bmi160_set_regs(reg_addr, & data, BMI160_ONE);
    }
    return rslt;
}
int8_t bmi160_set_fifo_config(uint8_t config, uint8_t enable) {
    ...
    if (dev == NULL) {
        ...
        rslt = bmi160_get_regs(reg_addr, & data, BMI160_ONE);
        if (rslt == BMI160_OK) {
            if (fifo_config > 0) {
...
int8_t bmi160_extract_accel(struct bmi160_sensor_data * accel_data, uint8_t * accel_length) {
    ...
    if (dev == NULL || dev -> fifo == NULL || dev -> fifo -> data == NULL) {
        ...
        if (dev -> fifo -> fifo_header_enable == 0) {
            get_accel_len_to_parse( & data_index, & data_read_length, accel_length);
            for (; data_index < data_read_length;) {
                check_frame_validity( & data_index);
                fifo_data_enable = dev -> fifo -> fifo_data_enable;
                unpack_accel_frame(accel_data, & data_index, & accel_index, fifo_data_enable);
            }
            * accel_length = accel_index;
            dev -> fifo -> accel_byte_start_idx = data_index;
        } else {
            extract_accel_header_mode(accel_data, accel_length);
        }
    }
    return rslt;
}
int8_t bmi160_extract_gyro(struct bmi160_sensor_data * gyro_data, uint8_t * gyro_length) {
    ...
    if (dev == NULL || dev -> fifo -> data == NULL) {
        ...
        if (dev -> fifo -> fifo_header_enable == 0) {
            get_gyro_len_to_parse( & data_index, & data_read_length, gyro_length);
            for (; data_index < data_read_length;) {
                check_frame_validity( & data_index);
                fifo_data_enable = dev -> fifo -> fifo_data_enable;
            ...
            * gyro_length = gyro_index;
            ...
            extract_gyro_header_mode(gyro_data, gyro_length);
        }
    }
    return rslt;
}
...
static int8_t set_accel_pwr() {
    ...
    if ((dev -> accel_cfg.power >= BMI160_ACCEL_SUSPEND_MODE) && (dev -> accel_cfg.power <= BMI160_ACCEL_LOWPOWER_MODE)) {
        if (dev -> accel_cfg.power != dev -> prev_accel_cfg.power) {
            ...
            if (rslt == BMI160_OK) {
                rslt = bmi160_set_regs(BMI160_COMMAND_REG_ADDR, & dev -> accel_cfg.power, 1);
...
static void reset_fifo_data_structure() {
...
static int8_t get_fifo_byte_counter(uint16_t * bytes_to_read) {
    ...
    rslt |= bmi160_get_regs(addr, data, 2);
...
static void unpack_accel_frame(struct bmi160_sensor_data * acc, uint16_t * idx, uint8_t * acc_idx, uint8_t frame_info) {
    ...
    case BMI160_FIFO_A_ENABLE:
        ...
        unpack_accel_data( & acc[ * acc_idx], * idx);
    ...
    case BMI160_FIFO_G_A_ENABLE:
        ...
        unpack_accel_data( & acc[ * acc_idx], * idx + BMI160_FIFO_G_LENGTH);
    ...
    case BMI160_FIFO_M_A_ENABLE:
        ...
        unpack_accel_data( & acc[ * acc_idx], * idx + BMI160_FIFO_M_LENGTH);
    ...
    case BMI160_FIFO_M_G_A_ENABLE:
        ...
        unpack_accel_data( & acc[ * acc_idx], * idx + BMI160_FIFO_MG_LENGTH);
...
static void extract_accel_header_mode(struct bmi160_sensor_data * accel_data, uint8_t * accel_length) {
    ...
    for (data_index = dev -> fifo -> accel_byte_start_idx; data_index < dev -> fifo -> length;) {
        ...
        case BMI160_FIFO_HEAD_M_G_A:
            ...
            break;
        case BMI160_FIFO_HEAD_M:
            move_next_frame( & data_index, BMI160_FIFO_M_LENGTH);
            break;
        case BMI160_FIFO_HEAD_G:
            move_next_frame( & data_index, BMI160_FIFO_G_LENGTH);
            break;
        case BMI160_FIFO_HEAD_M_G:
            move_next_frame( & data_index, BMI160_FIFO_MG_LENGTH);
            break;
...
static void unpack_gyro_frame(struct bmi160_sensor_data * gyro, uint16_t * idx, uint8_t * gyro_idx, uint8_t frame_info) {
    ...
    case BMI160_FIFO_G_ENABLE:
        ...
        unpack_gyro_data( & gyro[ * gyro_idx], * idx);
        ( * idx) = ( * idx) + BMI160_FIFO_G_LENGTH;
        ( * gyro_idx) ++;
        break;
    case BMI160_FIFO_HEAD_G_A:
    ...
    case BMI160_FIFO_M_G_A_ENABLE:
        ...
        unpack_gyro_data( & gyro[ * gyro_idx], * idx + BMI160_FIFO_M_LENGTH);
        * idx = * idx + BMI160_FIFO_MGA_LENGTH;
        ( * gyro_idx) ++;
        break;
    case BMI160_FIFO_HEAD_M_A:
...
static void extract_gyro_header_mode(struct bmi160_sensor_data * gyro_data, uint8_t * gyro_length) {
    ...
    for (data_index = dev -> fifo -> gyro_byte_start_idx; data_index < dev -> fifo -> length;) {
        ...
        case BMI160_FIFO_HEAD_M_G_A:
            ...
            break;
        case BMI160_FIFO_HEAD_A:
            move_next_frame( & data_index, BMI160_FIFO_A_LENGTH);
            break;
        case BMI160_FIFO_HEAD_M:
            move_next_frame( & data_index, BMI160_FIFO_M_LENGTH);
            break;
...
int8_t bmi160_set_step_counter(uint8_t step_cnt_enable) {
    ...
    if (rslt != BMI160_OK) {
        ...
        rslt = bmi160_get_regs(BMI160_INT_STEP_CONFIG_1_ADDR, & data, 1);
        if (rslt == BMI160_OK) {
            ...
            rslt = bmi160_set_regs(BMI160_INT_STEP_CONFIG_1_ADDR, & data, 1);
        }
    }
    return rslt;
}
int8_t bmi160_read_step_counter(uint16_t * step_val) {
    ...
    if (rslt != BMI160_OK) {
        ...
        rslt = bmi160_get_regs(BMI160_INT_STEP_CNT_0_ADDR, data, 2);
        if (rslt == BMI160_OK) {
            lsb = data[0];
            msb = data[1] << 8;
            * step_val = msb | lsb;
...
static int8_t config_aux_odr();
static int8_t config_aux_settg();
static int8_t extract_aux_read(uint16_t map_len, uint8_t reg_addr, uint8_t * aux_data, uint16_t len);
#endif
#ifdef BMI_TEST_TYPE
static int8_t perform_accel_self_test();
static int8_t enable_accel_self_test();
...
static int8_t perform_gyro_self_test();
static int8_t enable_gyro_self_test();
static int8_t validate_gyro_self_test();
#endif
#ifdef BMI_INT_TYPE
int8_t bmi160_set_int_config(struct bmi160_int_settg * int_config) {
    ...
    case BMI160_ACC_ANY_MOTION_INT:
        rslt = set_accel_any_motion_int(int_config);
        break;
    case BMI160_ACC_SIG_MOTION_INT:
        rslt = set_accel_sig_motion_int(int_config);
        break;
    case BMI160_ACC_SLOW_NO_MOTION_INT:
        rslt = set_accel_no_motion_int(int_config);
        break;
    case BMI160_ACC_DOUBLE_TAP_INT:
    case BMI160_ACC_SINGLE_TAP_INT:
        rslt = set_accel_tap_int(int_config);
        break;
    case BMI160_STEP_DETECT_INT:
        rslt = set_accel_step_detect_int(int_config);
        break;
    case BMI160_ACC_ORIENT_INT:
        rslt = set_accel_orientation_int(int_config);
        break;
    case BMI160_ACC_FLAT_INT:
        rslt = set_accel_flat_detect_int(int_config);
        break;
    case BMI160_ACC_LOW_G_INT:
        rslt = set_accel_low_g_int(int_config);
        break;
    case BMI160_ACC_HIGH_G_INT:
        rslt = set_accel_high_g_int(int_config);
        break;
    case BMI160_ACC_GYRO_DATA_RDY_INT:
        rslt = set_accel_gyro_data_ready_int(int_config);
        break;
    case BMI160_ACC_GYRO_FIFO_FULL_INT:
        rslt = set_fifo_full_int(int_config);
        break;
    case BMI160_ACC_GYRO_FIFO_WATERMARK_INT:
        rslt = set_fifo_watermark_int(int_config);
        break;
    case BMI160_FIFO_TAG_INT_PIN:
        rslt = set_intr_pin_config(int_config);
        break;
    default:
        break;
    }
...
int8_t bmi160_aux_read(uint8_t reg_addr, uint8_t * aux_data, uint16_t len) {
    ...
    if ((dev == NULL) || (dev -> read == NULL)) {
        ...
        if (dev -> aux_cfg.aux_sensor_enable == BMI160_ENABLE) {
            rslt = map_read_len( & map_len);
            if (rslt == BMI160_OK) {
                rslt = extract_aux_read(map_len, reg_addr, aux_data, len);
            }
        } else {
            rslt = BMI160_E_INVALID_INPUT;
        }
...
int8_t bmi160_aux_write(uint8_t reg_addr, uint8_t * aux_data, uint16_t len) {
    ...
    if ((dev == NULL) || (dev -> write == NULL)) {
        ...
        for (; count < len; count++) {
            rslt = bmi160_set_regs(BMI160_AUX_IF_4_ADDR, aux_data, 1);
            dev -> delay_ms(BMI160_AUX_COM_DELAY);
            if (rslt == BMI160_OK) {
                rslt = bmi160_set_regs(BMI160_AUX_IF_3_ADDR, & reg_addr, 1);
                dev -> delay_ms(BMI160_AUX_COM_DELAY);
...
int8_t bmi160_set_aux_auto_mode(uint8_t * data_addr) {
    ...
    if (rslt != BMI160_OK) {
        ...
        if (dev -> aux_cfg.aux_sensor_enable == BMI160_ENABLE) {
            rslt = bmi160_set_regs(BMI160_AUX_IF_2_ADDR, data_addr, 1);
            dev -> delay_ms(BMI160_AUX_COM_DELAY);
            if (rslt == BMI160_OK) {
                rslt = config_aux_odr();
                if (rslt == BMI160_OK) {
                    dev -> aux_cfg.manual_enable = BMI160_DISABLE;
                    rslt = bmi160_config_aux_mode();
                }
            }
        } else {
            rslt = BMI160_E_INVALID_INPUT;
...
int8_t bmi160_config_aux_mode() {
    int8_t rslt;
    uint8_t aux_if[2] = {
        (uint8_t)(dev -> aux_cfg.aux_i2c_addr * 2),
        0
    };
    rslt = bmi160_get_regs(BMI160_AUX_IF_1_ADDR, & aux_if[1], 1);
    if (rslt == BMI160_OK) {
        ...
        rslt = bmi160_set_regs(BMI160_AUX_IF_0_ADDR, & aux_if[0], 2);
        dev -> delay_ms(BMI160_AUX_COM_DELAY);
    }
    return rslt;
}
int8_t bmi160_read_aux_data_auto_mode(uint8_t * aux_data) {
    ...
    if (rslt != BMI160_OK) {
        ...
        if ((dev -> aux_cfg.aux_sensor_enable == BMI160_ENABLE) && (dev -> aux_cfg.manual_enable == BMI160_DISABLE)) {
            rslt = bmi160_get_regs(BMI160_AUX_DATA_ADDR, aux_data, 8);
        } else {
            rslt = BMI160_E_INVALID_INPUT;
        }
    }
...
int8_t bmi160_extract_aux(struct bmi160_aux_data * aux_data, uint8_t * aux_len) {
    ...
    if ((dev == NULL) || (dev -> fifo -> data == NULL) || (aux_data == NULL)) {
        ...
        if (dev -> fifo -> fifo_header_enable == 0) {
            get_aux_len_to_parse( & data_index, & data_read_length, aux_len);
            for (; data_index < data_read_length;) {
                check_frame_validity( & data_index);
                fifo_data_enable = dev -> fifo -> fifo_data_enable;
            ...
            * aux_len = aux_index;
            ...
            extract_aux_header_mode(aux_data, aux_len);
        }
    }
    return rslt;
}
...
int8_t bmi160_get_int_status(enum bmi160_int_status_sel int_status_sel, union bmi160_int_status * int_status, ) {
    ...
    if (int_status_sel == BMI160_INT_STATUS_ALL) {
        rslt = bmi160_get_regs(BMI160_INT_STATUS_ADDR, & int_status -> data[0], 4);
    } else {
        if (int_status_sel & BMI160_INT_STATUS_0) {
            rslt = bmi160_get_regs(BMI160_INT_STATUS_ADDR, & int_status -> data[0], 1);
        }
        if (int_status_sel & BMI160_INT_STATUS_1) {
            rslt = bmi160_get_regs(BMI160_INT_STATUS_ADDR + 1, & int_status -> data[1], 1);
        }
        if (int_status_sel & BMI160_INT_STATUS_2) {
            rslt = bmi160_get_regs(BMI160_INT_STATUS_ADDR + 2, & int_status -> data[2], 1);
        }
        if (int_status_sel & BMI160_INT_STATUS_3) {
            rslt = bmi160_get_regs(BMI160_INT_STATUS_ADDR + 3, & int_status -> data[3], 1);
        }
    }
    return rslt;
}
static int8_t set_accel_any_motion_int(struct bmi160_int_settg * int_config) {
    ...
    if ((rslt != BMI160_OK) || (int_config == NULL)) {
        ...
        rslt = enable_accel_any_motion_int(any_motion_int_cfg);
        if (rslt == BMI160_OK) {
            rslt = config_any_motion_int_settg(int_config, any_motion_int_cfg);
        }
    }
    return rslt;
}
static int8_t set_accel_tap_int(struct bmi160_int_settg * int_config) {
    ...
    if ((rslt != BMI160_OK) || (int_config == NULL)) {
        ...
        rslt = enable_tap_int(int_config, tap_int_cfg);
        if (rslt == BMI160_OK) {
            rslt = set_intr_pin_config(int_config);
            if (rslt == BMI160_OK) {
                rslt = config_tap_int_settg(int_config, tap_int_cfg);
            }
        }
    }
    return rslt;
...
static int8_t set_accel_gyro_data_ready_int(const struct bmi160_int_settg * int_config) {
    ...
    if ((rslt != BMI160_OK) || (int_config == NULL)) {
        ...
        rslt = enable_data_ready_int();
        if (rslt == BMI160_OK) {
            rslt = set_intr_pin_config(int_config);
            if (rslt == BMI160_OK) {
                rslt = map_hardware_interrupt(int_config);
            }
        }
    }
    return rslt;
...
static int8_t set_accel_sig_motion_int(struct bmi160_int_settg * int_config) {
    ...
    if ((rslt != BMI160_OK) || (int_config == NULL)) {
        ...
        rslt = enable_sig_motion_int(sig_mot_int_cfg);
        if (rslt == BMI160_OK) {
            rslt = config_sig_motion_int_settg(int_config, sig_mot_int_cfg);
        }
    }
    return rslt;
}
static int8_t set_accel_no_motion_int(struct bmi160_int_settg * int_config) {
    ...
    if ((rslt != BMI160_OK) || (int_config == NULL)) {
        ...
        rslt = enable_no_motion_int(no_mot_int_cfg);
        if (rslt == BMI160_OK) {
            rslt = config_no_motion_int_settg(int_config, no_mot_int_cfg);
        }
    }
    return rslt;
}
static int8_t set_accel_step_detect_int(struct bmi160_int_settg * int_config) {
    ...
    if ((rslt != BMI160_OK) || (int_config == NULL)) {
        ...
        rslt = enable_step_detect_int(step_detect_int_cfg);
        if (rslt == BMI160_OK) {
            rslt = set_intr_pin_config(int_config);
            if (rslt == BMI160_OK) {
                rslt = map_feature_interrupt(int_config);
                if (rslt == BMI160_OK) {
                    rslt = config_step_detect(step_detect_int_cfg);
                }
            }
        }
    }
...
static int8_t set_accel_orientation_int(struct bmi160_int_settg * int_config) {
    ...
    if ((rslt != BMI160_OK) || (int_config == NULL)) {
        ...
        rslt = enable_orient_int(orient_int_cfg);
        if (rslt == BMI160_OK) {
            rslt = set_intr_pin_config(int_config);
            if (rslt == BMI160_OK) {
                rslt = map_feature_interrupt(int_config);
                if (rslt == BMI160_OK) {
                    rslt = config_orient_int_settg(orient_int_cfg);
                }
            }
        }
    }
...
static int8_t set_accel_flat_detect_int(struct bmi160_int_settg * int_config) {
    ...
    if ((rslt != BMI160_OK) || (int_config == NULL)) {
        ...
        rslt = enable_flat_int(flat_detect_int);
        if (rslt == BMI160_OK) {
            rslt = set_intr_pin_config(int_config);
            if (rslt == BMI160_OK) {
                rslt = map_feature_interrupt(int_config);
                if (rslt == BMI160_OK) {
                    rslt = config_flat_int_settg(flat_detect_int);
                }
            }
        }
    }
...
static int8_t set_accel_low_g_int(struct bmi160_int_settg * int_config) {
    ...
    if ((rslt != BMI160_OK) || (int_config == NULL)) {
        ...
        rslt = enable_low_g_int(low_g_int);
        if (rslt == BMI160_OK) {
            rslt = set_intr_pin_config(int_config);
            if (rslt == BMI160_OK) {
                rslt = map_feature_interrupt(int_config);
                if (rslt == BMI160_OK) {
                    rslt = config_low_g_data_src(low_g_int);
                    if (rslt == BMI160_OK) {
                        rslt = config_low_g_int_settg(low_g_int);
                    }
                }
            }
        }
...
static int8_t set_accel_high_g_int(struct bmi160_int_settg * int_config) {
    ...
    if ((rslt != BMI160_OK) || (int_config == NULL)) {
        ...
        rslt = enable_high_g_int(high_g_int_cfg);
        if (rslt == BMI160_OK) {
            rslt = set_intr_pin_config(int_config);
            if (rslt == BMI160_OK) {
                rslt = map_feature_interrupt(int_config);
                if (rslt == BMI160_OK) {
                    rslt = config_high_g_data_src(high_g_int_cfg);
                    if (rslt == BMI160_OK) {
                        rslt = config_high_g_int_settg(high_g_int_cfg);
                    }
                }
            }
        }
...
static int8_t set_intr_pin_config(const struct bmi160_int_settg * int_config) {
    int8_t rslt;
    rslt = config_int_out_ctrl(int_config);
    if (rslt == BMI160_OK) {
        rslt = config_int_latch(int_config);
    }
    return rslt;
}
#endif
...
static int8_t enable_accel_any_motion_int(const struct bmi160_acc_any_mot_int_cfg * any_motion_int_cfg, ) {
    ...
    uint8_t data = 0;
    uint8_t temp = 0;
    rslt = bmi160_get_regs(BMI160_INT_ENABLE_0_ADDR, & data, 1);
    if (rslt == BMI160_OK) {
        ...
        rslt = bmi160_set_regs(BMI160_INT_ENABLE_0_ADDR, & data, 1);
    }
    return rslt;
}
static int8_t disable_sig_motion_int() {
    ...
    uint8_t data = 0;
    uint8_t temp = 0;
    rslt = bmi160_get_regs(BMI160_INT_MOTION_3_ADDR, & data, 1);
    if (rslt == BMI160_OK) {
        temp = (data & BMI160_SIG_MOTION_SEL_MASK);
        if (temp) {
            temp = data & ~BMI160_SIG_MOTION_SEL_MASK;
            data = temp;
            rslt = bmi160_set_regs(BMI160_INT_MOTION_3_ADDR, & data, 1);
        }
    }
    return rslt;
}
static int8_t map_feature_interrupt(const struct bmi160_int_settg * int_config) {
    ...
    rslt = bmi160_get_regs(BMI160_INT_MAP_0_ADDR, data, 3);
    if (rslt == BMI160_OK) {
        ...
        if (rslt == BMI160_OK) {
            rslt = bmi160_set_regs(BMI160_INT_MAP_0_ADDR, data, 3);
        }
    }
    return rslt;
}
static int8_t map_hardware_interrupt(const struct bmi160_int_settg * int_config) {
    ...
    uint8_t data = 0;
    uint8_t temp = 0;
    rslt = bmi160_get_regs(BMI160_INT_MAP_1_ADDR, & data, 1);
    if (rslt == BMI160_OK) {
        ...
        if (rslt == BMI160_OK) {
            rslt = bmi160_set_regs(BMI160_INT_MAP_1_ADDR, & data, 1);
        }
    }
    return rslt;
}
...
static int8_t config_any_dur_threshold(const struct bmi160_acc_any_mot_int_cfg * any_motion_int_cfg) {
    ...
    uint8_t dur;
    rslt = bmi160_get_regs(BMI160_INT_MOTION_0_ADDR, & data, 1);
    if (rslt == BMI160_OK) {
        ...
        rslt = bmi160_set_regs(BMI160_INT_MOTION_0_ADDR, data_array, 2);
    }
    return rslt;
}
static int8_t config_any_motion_int_settg(const struct bmi160_int_settg * int_config,
    ...
    rslt = set_intr_pin_config(int_config);
    if (rslt == BMI160_OK) {
        rslt = disable_sig_motion_int();
        if (rslt == BMI160_OK) {
            rslt = map_feature_interrupt(int_config);
            if (rslt == BMI160_OK) {
                rslt = config_any_motion_src(any_motion_int_cfg);
                if (rslt == BMI160_OK) {
                    rslt = config_any_dur_threshold(any_motion_int_cfg);
                }
            }
        }
    }
...
static int8_t enable_data_ready_int() {
    ...
    uint8_t data = 0;
    uint8_t temp = 0;
    rslt = bmi160_get_regs(BMI160_INT_ENABLE_1_ADDR, & data, 1);
    if (rslt == BMI160_OK) {
        temp = data & ~BMI160_DATA_RDY_INT_EN_MASK;
        data = temp | ((1 << 4) & BMI160_DATA_RDY_INT_EN_MASK);
        rslt = bmi160_set_regs(BMI160_INT_ENABLE_1_ADDR, & data, 1);
    }
    return rslt;
}
static int8_t enable_no_motion_int(const struct bmi160_acc_no_motion_int_cfg * no_mot_int_cfg) {
    ...
    uint8_t data = 0;
    uint8_t temp = 0;
    rslt = bmi160_get_regs(BMI160_INT_ENABLE_2_ADDR, & data, 1);
    if (rslt == BMI160_OK) {
        ...
        rslt = bmi160_set_regs(BMI160_INT_ENABLE_2_ADDR, & data, 1);
    }
    return rslt;
}
static int8_t config_no_motion_int_settg(const struct bmi160_int_settg * int_config,
    ...
    if (rslt == BMI160_OK) {
        rslt = map_feature_interrupt(int_config);
        if (rslt == BMI160_OK) {
            rslt = config_no_motion_data_src(no_mot_int_cfg);
            if (rslt == BMI160_OK) {
                rslt = config_no_motion_dur_thr(no_mot_int_cfg);
            }
        }
    }
    return rslt;
    ...
    if (rslt == BMI160_OK) {
        ...
        if (rslt == BMI160_OK) {
            ...
            if (rslt == BMI160_OK) {
                ...
                rslt = bmi160_set_regs(reg_addr, data_array, 2);
            }
        }
    }
    return rslt;
...
static int8_t config_sig_motion_int_settg(const struct bmi160_int_settg * int_config,
    ...
    if (rslt == BMI160_OK) {
        rslt = map_feature_interrupt(int_config);
        if (rslt == BMI160_OK) {
            rslt = config_sig_motion_data_src(sig_mot_int_cfg);
            if (rslt == BMI160_OK) {
                rslt = config_sig_dur_threshold(sig_mot_int_cfg);
            }
        }
    }
    return rslt;
...
static int8_t config_sig_dur_threshold(const struct bmi160_acc_sig_mot_int_cfg * sig_mot_int_cfg) {
    ...
    rslt = bmi160_set_regs(BMI160_INT_MOTION_1_ADDR, & data, 1);
    if (rslt == BMI160_OK) {
        rslt = bmi160_get_regs(BMI160_INT_MOTION_3_ADDR, & data, 1);
        if (rslt == BMI160_OK) {
...
static int8_t config_step_detect(const struct bmi160_acc_step_detect_int_cfg * step_detect_int_cfg) {
    ...
    if (step_detect_int_cfg -> step_detector_mode == BMI160_STEP_DETECT_NORMAL) {
        ...
        rslt = bmi160_get_regs(BMI160_INT_STEP_CONFIG_0_ADDR, & data_array[0], 2);
        if (rslt == BMI160_OK) {
    ...
    rslt = bmi160_set_regs(BMI160_INT_STEP_CONFIG_0_ADDR, data_array, 2);
    return rslt;
}
static int8_t enable_tap_int(const struct bmi160_int_settg * int_config,
    const struct bmi160_acc_tap_int_cfg * tap_int_cfg) {
...
static int8_t config_tap_int_settg(const struct bmi160_int_settg * int_config,
    ...
    if (rslt == BMI160_OK) {
        rslt = map_feature_interrupt(int_config);
        if (rslt == BMI160_OK) {
            rslt = config_tap_data_src(tap_int_cfg);
            if (rslt == BMI160_OK) {
                rslt = config_tap_param(int_config, tap_int_cfg);
            }
        }
    }
    return rslt;
...
static int8_t config_tap_data_src(const struct bmi160_acc_tap_int_cfg * tap_int_cfg) {
    ...
    uint8_t data = 0;
    uint8_t temp = 0;
    rslt = bmi160_get_regs(BMI160_INT_DATA_0_ADDR, & data, 1);
    if (rslt == BMI160_OK) {
        temp = data & ~BMI160_TAP_SRC_INT_MASK;
        data = temp | ((tap_int_cfg -> tap_data_src << 3) & BMI160_TAP_SRC_INT_MASK);
        rslt = bmi160_set_regs(BMI160_INT_DATA_0_ADDR, & data, 1);
    }
    return rslt;
}
static int8_t config_tap_param(const struct bmi160_int_settg * int_config,
    ...
    rslt = bmi160_get_regs(BMI160_INT_TAP_0_ADDR, data_array, 2);
    if (rslt == BMI160_OK) {
        ...
        data_array[count++] = data;
        rslt = bmi160_set_regs(BMI160_INT_TAP_0_ADDR, data_array, count);
    }
    return rslt;
}
static int8_t config_sec_if() {
    ...
    if (rslt == BMI160_OK) {
        dev -> delay_ms(1);
        rslt = bmi160_get_regs(BMI160_IF_CONF_ADDR, & if_conf, 1);
        if_conf |= (uint8_t)(1 << 5);
        if (rslt == BMI160_OK) {
            rslt = bmi160_set_regs(BMI160_IF_CONF_ADDR, & if_conf, 1);
        }
    }
    return rslt;
}
...
static int8_t config_aux_odr() {
    int8_t rslt;
    uint8_t aux_odr;
    rslt = bmi160_get_regs(BMI160_AUX_ODR_ADDR, & aux_odr, 1);
    if (rslt == BMI160_OK) {
        aux_odr = (uint8_t)(dev -> aux_cfg.aux_odr);
        rslt = bmi160_set_regs(BMI160_AUX_ODR_ADDR, & aux_odr, 1);
        dev -> delay_ms(BMI160_AUX_COM_DELAY);
    }
    return rslt;
}
...
static int8_t extract_aux_read(uint16_t map_len, uint8_t reg_addr, uint8_t * aux_data, uint16_t len) {
    ...
    for (; count < len;) {
        rslt = bmi160_set_regs(BMI160_AUX_IF_2_ADDR, & reg_addr, 1);
        dev -> delay_ms(BMI160_AUX_COM_DELAY);
        if (rslt == BMI160_OK) {
            rslt = bmi160_get_regs(read_addr, data, map_len);
            if (rslt == BMI160_OK) {
                read_count = 0;
                if (len < map_len) {
...
static int8_t config_orient_int_settg(const struct bmi160_acc_orient_int_cfg * orient_int_cfg) {
    ...
    rslt = bmi160_get_regs(BMI160_INT_ORIENT_0_ADDR, data_array, 2);
    if (rslt == BMI160_OK) {
        data = data_array[0];
        ...
        data_array[1] = data;
        rslt = bmi160_set_regs(BMI160_INT_ORIENT_0_ADDR, data_array, 2);
    }
    return rslt;
}
static int8_t enable_flat_int(const struct bmi160_acc_flat_detect_int_cfg * flat_int) {
...
static int8_t config_flat_int_settg(const struct bmi160_acc_flat_detect_int_cfg * flat_int) {
    ...
    rslt = bmi160_get_regs(BMI160_INT_FLAT_0_ADDR, data_array, 2);
    if (rslt == BMI160_OK) {
        data = data_array[0];
        ...
        data_array[1] = data;
        rslt = bmi160_set_regs(BMI160_INT_FLAT_0_ADDR, data_array, 2);
    }
    return rslt;
}
static int8_t enable_low_g_int(const struct bmi160_acc_low_g_int_cfg * low_g_int) {
...
static int8_t config_low_g_int_settg(const struct bmi160_acc_low_g_int_cfg * low_g_int) {
    ...
    rslt = bmi160_get_regs(BMI160_INT_LOWHIGH_2_ADDR, & data_array[2], 1);
    if (rslt == BMI160_OK) {
        ...
        rslt = bmi160_set_regs(BMI160_INT_LOWHIGH_0_ADDR, data_array, 3);
    }
    return rslt;
}
static int8_t enable_high_g_int(const struct bmi160_acc_high_g_int_cfg * high_g_int_cfg) {
...
static int8_t config_high_g_int_settg(const struct bmi160_acc_high_g_int_cfg * high_g_int_cfg) {
    ...
    rslt = bmi160_get_regs(BMI160_INT_LOWHIGH_2_ADDR, & data_array[0], 1);
    if (rslt == BMI160_OK) {
        ...
        rslt = bmi160_set_regs(BMI160_INT_LOWHIGH_2_ADDR, data_array, 3);
    }
    return rslt;
}
static int8_t config_int_out_ctrl(const struct bmi160_int_settg * int_config) {
    ...
    uint8_t temp = 0;
    uint8_t data = 0;
    rslt = bmi160_get_regs(BMI160_INT_OUT_CTRL_ADDR, & data, 1);
    if (rslt == BMI160_OK) {
        ...
        rslt = bmi160_set_regs(BMI160_INT_OUT_CTRL_ADDR, & data, 1);
    }
    return rslt;
}
static int8_t config_int_latch(const struct bmi160_int_settg * int_config) {
    ...
    uint8_t temp = 0;
    uint8_t data = 0;
    rslt = bmi160_get_regs(BMI160_INT_LATCH_ADDR, & data, 1);
    if (rslt == BMI160_OK) {
        const struct bmi160_int_pin_settg * intr_pin_sett = & (int_config -> int_pin_settg);
        if (int_config -> int_channel == BMI160_INT_CHANNEL_1) {
        ...
        rslt = bmi160_set_regs(BMI160_INT_LATCH_ADDR, & data, 1);
    }
    return rslt;
}
#endif
...
int8_t bmi160_perform_self_test(uint8_t select_sensor) {
    ...
    if (rslt != BMI160_OK) {
        ...
        case BMI160_ACCEL_ONLY:
            rslt = perform_accel_self_test();
            break;
        case BMI160_GYRO_ONLY:
            dev -> gyro_cfg.power = BMI160_GYRO_NORMAL_MODE;
            rslt = bmi160_set_power_mode();
            if (rslt == BMI160_OK) {
                rslt = perform_gyro_self_test();
            }
            break;
        default:
            rslt = BMI160_E_INVALID_INPUT;
...
static int8_t perform_accel_self_test() {
    ...
    rslt = enable_accel_self_test();
    if (rslt == BMI160_OK) {
        rslt = accel_self_test_positive_excitation( & accel_pos);
        if (rslt == BMI160_OK) {
            rslt = accel_self_test_negative_excitation( & accel_neg);
            if (rslt == BMI160_OK) {
...
static int8_t enable_accel_self_test() {
    ...
    if (rslt == BMI160_OK) {
        ...
        rslt = bmi160_set_regs(BMI160_ACCEL_CONFIG_ADDR, & reg_data, 1);
    }
    return rslt;
}
static int8_t accel_self_test_positive_excitation(struct bmi160_sensor_data * accel_pos) {
    ...
    rslt = bmi160_set_regs(BMI160_SELF_TEST_ADDR, & reg_data, 1);
    if (rslt == BMI160_OK) {
        dev -> delay_ms(BMI160_ACCEL_SELF_TEST_DELAY);
        rslt = bmi160_get_sensor_data(BMI160_ACCEL_ONLY, accel_pos, NULL);
    }
    return rslt;
}
static int8_t accel_self_test_negative_excitation(struct bmi160_sensor_data * accel_neg) {
    ...
    if (rslt == BMI160_OK) {
        dev -> delay_ms(BMI160_ACCEL_SELF_TEST_DELAY);
        rslt = bmi160_get_sensor_data(BMI160_ACCEL_ONLY, accel_neg, NULL);
    }
    return rslt;
}
static int8_t validate_accel_self_test(const struct bmi160_sensor_data * accel_pos,
...
static int8_t perform_gyro_self_test() {
    int8_t rslt;
    rslt = enable_gyro_self_test();
    if (rslt == BMI160_OK) {
        dev -> delay_ms(50);
        rslt = validate_gyro_self_test();
    }
    return rslt;
}
static int8_t enable_gyro_self_test() {
    int8_t rslt;
    uint8_t reg_data;
    rslt = bmi160_get_regs(BMI160_SELF_TEST_ADDR, & reg_data, 1);
    if (rslt == BMI160_OK) {
...
static int8_t validate_gyro_self_test() {
    int8_t rslt;
    uint8_t reg_data;
    rslt = bmi160_get_regs(BMI160_STATUS_ADDR, & reg_data, 1);
    if (rslt == BMI160_OK) {
...
static int8_t set_fifo_full_int(const struct bmi160_int_settg * int_config) {
    ...
    if ((dev == NULL) || (dev -> delay_ms == NULL)) {
        ...
        rslt = enable_fifo_full_int(int_config);
        if (rslt == BMI160_OK) {
            rslt = set_intr_pin_config(int_config);
            if (rslt == BMI160_OK) {
                rslt = map_hardware_interrupt(int_config);
...
static int8_t set_fifo_watermark_int(const struct bmi160_int_settg * int_config) {
    ...
    if ((dev == NULL) || (dev -> delay_ms == NULL)) {
        ...
        rslt = enable_fifo_wtm_int(int_config);
        if (rslt == BMI160_OK) {
            rslt = set_intr_pin_config(int_config);
            if (rslt == BMI160_OK) {
                rslt = map_hardware_interrupt(int_config);
...
static void unpack_aux_frame(struct bmi160_aux_data * aux_data, uint16_t * idx, uint8_t * aux_index, uint8_t frame_info) {
    ...
    case BMI160_FIFO_M_ENABLE:
        ...
        unpack_aux_data( & aux_data[ * aux_index], * idx);
        * idx = * idx + BMI160_FIFO_M_LENGTH;
        ( * aux_index) ++;
        break;
    case BMI160_FIFO_HEAD_M_A:
...
static void extract_aux_header_mode(struct bmi160_aux_data * aux_data, uint8_t * aux_length) {
    ...
    for (data_index = dev -> fifo -> aux_byte_start_idx; data_index < dev -> fifo -> length;) {
        ...
        case BMI160_FIFO_HEAD_M_G:
        case BMI160_FIFO_HEAD_M_G_A:
            unpack_aux_frame(aux_data, & data_index, & aux_index, frame_header);
            break;
        case BMI160_FIFO_HEAD_G:
            move_next_frame( & data_index, BMI160_FIFO_G_LENGTH);
            break;
        case BMI160_FIFO_HEAD_G_A:
            move_next_frame( & data_index, BMI160_FIFO_GA_LENGTH);
            break;
        case BMI160_FIFO_HEAD_A:
            move_next_frame( & data_index, BMI160_FIFO_A_LENGTH);
            break;
